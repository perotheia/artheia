"""Node prototype inheritance: ``node X prototype Y { tipc ... }``.

textX parses the prototype clause as a cross-ref; the loader's
...
Covers:
  ...
  - Chains: A prototype B prototype C
...
def test_chain_inheritance():
    """A prototype B prototype C — A absorbs C's fields transitively."""
    m = parse_string(
        """
        package test.inh
        interface senderReceiver Stream { }
        ...
        node atomic B prototype C {
            tipc type=0x10010002 instance=0
        }
        node atomic A prototype B {
...
def test_cycle_detected():
    ...
    with pytest.raises(ValueError, match="prototype cycle"):
        parse_string(
            """
            package test.inh
            node atomic A prototype B {
...
def test_no_extends_unchanged():
    """A node WITHOUT prototype behaves identically to before the flatten
