...
_LIB_BUILD_BAZEL_TEMPLATE = """\
# AUTO-GENERATED by `artheia gen-fc-lib --vendored` — DO NOT EDIT.
# Self-contained VENDORED node-lib slice for {fc_short} ({source_file}).
#
# Builds standalone as a Bazel artifact — vendors platform/runtime/ under
# runtime/ and the nanopb-compiled protos under generated/, so this repo
# compiles WITHOUT a workspace @pero_theia dep. An external CMake build (if
# any) drives `bazel build` from outside; there is no CMake tree here.
load("@rules_cc//cc:defs.bzl", "cc_library")

package(default_visibility = ["//visibility:public"])

# ── Vendored Theia runtime — runtime/src/*.cc + runtime/include ──────────
# The self-contained actor stack (GenServer/NodeRef/TimerService/TipcMux/
...
cc_library(
    ...
    hdrs = glob([
        "runtime/include/**/*.hh",
        "runtime/include/**/*.h",
        "generated/platform_runtime/*.pb.h",
    ]),
...
# nanopb-compiled at generate time (the .proto is workspace-only). A repo
# with no app proto still links (empty glob → header-only).
cc_library(
    name = "{fc_short}_protos",
    srcs = glob(
        ["generated/**/*.pb.c"],
        exclude = ["generated/platform_runtime/*.pb.c"],
    ),
...
# Per-node <Node>.hh + <Node>_netgraph.hh + FC-wide Log.hh / {fc_short}_codecs.hh.
# The headers reference each other BARE ("Log.hh", "{fc_short}_codecs.hh") AND by
# repo-root path ("lib/<Node>.hh", "impl/<Node>_state.hh"). `strip_include_prefix
# = "lib"` re-roots the bare ones; the repo-root ones resolve from the package
# root (the default include path), so no `includes = ["."]` (Bazel forbids that).
cc_library(
    name = "{fc_short}_lib",
    hdrs = glob(["lib/*.hh"]),
    strip_include_prefix = "lib",
    deps = [":{fc_short}_protos", ":platform_runtime"],
)

# ── Impl slice — the user-owned handler bodies ───────────────────────────
# Per-node *_handlers.cc (write-once). Link {fc_short}_impl from the app's own
# cc_binary (its main.cc — see main.example.cc) to get the whole stack. The impl
# textual_hdrs re-expose the state headers by their repo-root path.
cc_library(
    name = "{fc_short}_impl",
    srcs = [
{handler_srcs}
...
def _emit_lib_build(fc_short: str, source_file: str, handler_srcs: list[str]) -> str:
    """Render the BUILD.bazel for the vendored lib slice.

    handler_srcs is a list of `impl/<Node>_handlers.cc` paths relative to <out>.
    Listed explicitly (a bazel srcs list, not a glob) so a node-removal in .art
    fails the build visibly instead of silently shipping a stale handler.
    """
    lines = "\n".join(f'        "{p}",' for p in handler_srcs)
    return _LIB_BUILD_BAZEL_TEMPLATE.format(
        fc_short=fc_short,
...
def generate_lib(
    ...
    *,
    ...
    # BUILD.bazel — the vendored lib's self-contained Bazel targets
    # (platform_runtime + <fc>_protos + <fc>_lib + <fc>_impl). Always regen —
    # never user-edited (the app's own cc_binary links :<fc>_impl).
    p = out_dir / "BUILD.bazel"
    results[_write(p,
                   _emit_lib_build(mv.fc_short, str(art_path), handler_srcs),
                   overwrite=True)].append(str(p))
