    ...
    *,
    ...
    if proto_out is not None:
        ...
        if package_mode:
            ...
            # IMPORT PLUMBING (idempotence: derived from the emitted proto, not
            # hand-patched — hand patches were clobbered by every regen; see
            # theia docs/tasks/BACKLOG/artheia-package-proto-imports.md). A
            # platform.msgs embed (e.g. geometry.Vec2) makes the proto carry
            # `import "platform/msgs/<p>/<p>.proto"`: stage those framework
            # protos (+ std, the msgs base, for transitive imports) into the
            # genrule, add a second -I at the framework proto root, and dep the
            # cc_library on the platform_protos aggregate for the .pb.h.
            import re as _re
            _imports = _re.findall(r'^import "([^"]+)";',
                                   Path(proto_path).read_text(), _re.M)
            _plat = sorted({p.split("/")[2] for p in _imports
                            if p.startswith("platform/msgs/")})
            if _plat and "std" not in _plat:
                _plat.append("std")            # msgs base — transitive import
            _plat_srcs = "".join(
                f'            "@pero_theia//platform/proto/platform/msgs/{p}:{p}_proto",\n'
                f'            "@pero_theia//platform/proto/platform/msgs/{p}:{p}_options",\n'
                for p in _plat)
            if _plat:
                _p0 = _plat[0]
                # plain concatenation (no f-string): the emitted BUILD must
                # carry awk '{print $1}' and ${p0%/...} braces verbatim.
                _cmd = (
                    '    cmd = "set -e;"\n'
                    '        + " in_dir=$$(dirname $(location ' + _leaf + '.proto));"\n'
                    '        + " out_dir=$$(dirname $(location ' + _leaf + '.pb.c));"\n'
                    '        + " p0=$$(echo $(locations @pero_theia//platform/proto/platform/msgs/'
                    + _p0 + ':' + _p0 + "_proto) | awk '{print $$1}');\"\n"
                    '        + " theia_root=$${p0%/platform/msgs/'
                    + _p0 + '/' + _p0 + '.proto};"\n'
                    '        + " nanopb_generator -I $$in_dir -I $$theia_root -D $$out_dir '
                    + _leaf + '.proto;",\n')
                _deps = (
                    f'    deps = [\n'
                    f'        "@pero_theia//platform/runtime:runtime_proto_cc",\n'
                    f'        # platform.msgs .pb.h for the embedded common types\n'
                    f'        "@pero_theia//platform/proto:platform_protos",\n'
                    f'    ],\n')
            else:
                _cmd = (
                    f'    cmd = "set -e;"\n'
